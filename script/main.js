...
window.addEventListener( "DOMContentLoaded", () => {
    ...
    function onFormSubmit( e ) {
        e.preventDefault();
        const obj = [{}];
        const text_category = document.getElementById( 'form101' );
        const text_title = document.getElementById( 'form102' );
        const text_body = document.getElementById( 'form103' );

        tasks.length === 0 ? obj[0].id = 0: obj[0].id = (tasks[tasks.length-1].id + 1);
        obj[0].category = text_category.value;
        obj[0].title = text_title.value;
        obj[0].body = text_body.value;
